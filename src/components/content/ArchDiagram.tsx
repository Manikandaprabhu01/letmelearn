...
function NodeCard({ node }: { node: DiagramNode }) {
  return (
    ...
    >
      <div className="text-[13px] font-medium leading-snug">{node.label}</div>
      {node.sub ? <div className="mt-0.5 text-[11px] text-muted">{node.sub}</div> : null}
    </div>
  );
}
    ...
    <figure className="my-5 overflow-x-auto rounded-lg border border-border bg-inset p-4">
      <div className="flex min-w-[520px] overflow-hidden rounded-md border border-border">
        {diagram.fields.map((field) => (
          <div
            key={field.label}
            className="border-r border-border last:border-r-0 bg-raised px-2 py-3 text-center"
      ...
      <figcaption className="mt-3 text-center text-xs text-muted">
        {diagram.caption ?? `${total}-bit layout`}
      </figcaption>
    </figure>
  );
}
